...
- [x] **Read-side adapters shipped 2026-08-30** (`d917a35`). `atrium recall`
  ...
  `~/.claude/hooks/atrium-recall.sh` replaced the mempalace recall hook in
  `SessionStart` and was verified to load in *both* profiles by asking the
  Favish profile to quote the injected block back. The MCP server
...
- [x] **mempalace retired 2026-08-30**, reversibly -- every switch flipped, no
  ...
  * The `~/.claude` profile loads no stdio MCP server at all -- `atrium`,
    ...
    list, while `~/.claude-favish` loads them. Only plugin-provided servers
...
- [-] Folded into the retirement entry above. mempalace write side, measured 2026-08-30 -- smaller than assumed:
...
- [-] Folded into the retirement entry above. Reclaim: `~/.mempalace` is 118 GB, of which ~87 GB is dead rebuild
