...
`p/consumer-pv` holds 558 conversations from 2026-06-14 to 2026-07-10 and
`p/consumer-g` picks up on 2026-07-12 -- the same project, renamed. Without
...
def test_the_old_name_becomes_the_current_one():
    assert canonical_workspace("[HOME]/p/consumer-pv", HOME, ALIASES) == "[HOME]/p/consumer-g"


def test_subdirectories_of_the_old_name_move_too():
    """Worktrees and tool directories carry the old root as a prefix."""
    assert (
        canonical_workspace("[HOME]/p/consumer-pv/.playwright-mcp", HOME, ALIASES)
        == "[HOME]/p/consumer-g/.playwright-mcp"
    )
...
def test_a_missed_redaction_is_aliased_after_being_folded_home():
    ...
    assert canonical_workspace(f"{HOME}/p/consumer-pv", HOME, ALIASES) == "[HOME]/p/consumer-g"


def test_an_unrelated_project_is_untouched():
    assert canonical_workspace("[HOME]/p/project-after", HOME, ALIASES) == "[HOME]/p/project-after"


def test_a_name_prefix_neighbour_is_not_renamed():
    assert (
        canonical_workspace("[HOME]/p/consumer-pv-archive", HOME, ALIASES)
        == "[HOME]/p/consumer-pv-archive"
    )
...
def test_a_valid_file_is_read(tmp_path):
    ...
    assert workspace_aliases(path) == {"[HOME]/p/consumer-pv": WorkspaceAlias("[HOME]/p/consumer-g")}
