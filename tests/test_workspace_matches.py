...
def test_the_project_root_belongs_to_itself():
    assert workspace_matches("[HOME]/p/vexa", "[HOME]/p/vexa")


def test_a_subdirectory_belongs_to_the_project():
    assert workspace_matches("[HOME]/p/vexa/apps/web", "[HOME]/p/vexa")
...
def test_a_name_prefix_neighbour_does_not():
    ...
    one."""
    assert not workspace_matches("[HOME]/p/vexa-enrichment-backfill", "[HOME]/p/vexa")


def test_an_absent_workspace_matches_nothing():
    assert not workspace_matches(None, "[HOME]/p/vexa")
    assert not workspace_matches("", "[HOME]/p/vexa")


def test_a_trailing_slash_on_the_project_changes_nothing():
    assert workspace_matches("[HOME]/p/vexa/apps", "[HOME]/p/vexa/")
