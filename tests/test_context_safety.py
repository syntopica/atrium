...
def test_model_failure_degrades_explicitly_to_lexical(corpus):
    ...
    result = retrieve_context(
        connection, "Nova", project=project, lane="auto", state=state, embedder=BrokenEmbedder()
    )
...
def test_read_only_retrieval_does_not_modify_store(corpus):
    ...
    result = retrieve_context(reader, "Nova", project=project, lane="words", state=state)
    assert result["evidence"]
...
def test_missing_index_returns_structured_failure_and_creates_nothing(tmp_path):
    ...
    result = context_from_index(missing, "Nova", lane="words", state=tmp_path)
...
def test_malformed_instance_configuration_is_visible(tmp_path, monkeypatch):
    ...
    result = context_from_index(state / "index.sqlite3", "Nova", lane="words", state=state)
...
def test_lexical_plan_constrains_fts_rowids_before_reading_bodies(corpus):
    ...
    hits = lexical_hits(connection, "Nova", 8, "words", curated=False, workspace=str(project))
