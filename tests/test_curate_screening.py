...
def test_restating_where_the_session_ran_is_not_a_claim() -> None:
    """Measured: `Repository: ~/p/consumer-g` led the corpus at 59 episodes."""
    assert runtime_debris("Repository: `/Users/someone/p/consumer-g`.") == (
        "context_restatement"
    )
    assert runtime_debris("Project path: `/Users/someone/p/consumer-g`") == (
        "context_restatement"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Task report is documented at [HOME]/p/inbox-tool/sdd/task-7-report.md.",
            "path_pointer",
        ),
        (
...
@pytest.mark.parametrize(
    "fact",
    [
        "scripts/deploy-server-a.sh deploys the SaaS to a cPanel account via rsync to /home/x/build.",
