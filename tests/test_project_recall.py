...
def test_a_subdirectory_recalls_its_project_not_itself(tmp_path):
    ...
    home, root = _project(tmp_path, "p", "consumer-y")
    (root / ".git").mkdir()
    deep = root / "apps" / "web"
    deep.mkdir(parents=True)
    assert project_workspace(deep, home) == "[HOME]/p/consumer-y"
...
def test_both_worktree_layouts_belong_to_their_project(tmp_path):
    ...
    home2, sibling = _project(tmp_path, "p2", "client-talk.worktrees", "gha-build")
    (sibling / ".git").write_text("gitdir: elsewhere\n")
    assert project_workspace(sibling, home2) == "[HOME]/p2/client-talk"
