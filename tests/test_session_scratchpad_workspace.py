...
instead of being discarded -- vexa's 97 conversations under one such path are
vexa's. The 67 collapse to 19 real projects: 331 conversations remapped, 45
...
def home(tmp_path: Path) -> Path:
    ...
    (tmp_path / "p" / "vexa").mkdir(parents=True)
    (tmp_path / "p" / "inbox-companion").mkdir(parents=True)
...
def test_a_scratchpad_becomes_the_project_the_session_worked_in(home: Path):
    workspace = _scratchpad(home / "p" / "vexa")
    assert canonical_workspace(workspace, home) == "[HOME]/p/vexa"
...
def test_a_subdirectory_of_the_scratchpad_is_the_same_project(home: Path):
    ...
    workspace = _scratchpad(home / "p" / "vexa") + "/reading/group"
    assert canonical_workspace(workspace, home) == "[HOME]/p/vexa"


def test_a_hyphen_in_the_project_name_survives_the_encoding(home: Path):
    """`inbox-companion` and `inbox/companion` encode identically.
    ...
    real tree.
    """
    workspace = _scratchpad(home / "p" / "inbox-companion")
    assert canonical_workspace(workspace, home) == "[HOME]/p/inbox-companion"


def test_an_ambiguous_encoding_names_no_project(home: Path):
    """`inbox-companion` and `inbox/companion` cannot both be the answer.
...
def test_a_scratchpad_under_another_temp_root_is_still_a_scratchpad(home: Path):
    ...
    workspace = _scratchpad(home / "p" / "vexa", root="/tmp/claude-0")
    assert canonical_workspace(workspace, home) == "[HOME]/p/vexa"
...
def test_a_normal_workspace_is_untouched(home: Path):
    ...
    assert canonical_workspace(f"{home}/p/vexa", home) == "[HOME]/p/vexa"
    assert canonical_workspace("[HOME]/p/vexa", home) == "[HOME]/p/vexa"
...
def test_a_temp_path_that_is_not_a_scratchpad_is_untouched(home: Path):
    ...
    workspace = f"/private/tmp/claude-501/{_encoded(home / 'p' / 'vexa')}/{SESSION}/build"
...
def test_a_scratchpad_shape_outside_a_temp_root_is_untouched(home: Path):
    ...
    names -- here atrium's would become vexa's.
    """
    workspace = f"{home}/p/atrium/{_encoded(home / 'p' / 'vexa')}/{SESSION}/scratchpad"
...
def test_a_project_directory_that_is_a_symlink_still_decodes(tmp_path: Path) -> None:
    ...
    case, and `~/p/iriscaceres -> /Volumes/iris-cs/iriscaceres` is a live
    ...
    elsewhere = tmp_path / "volume" / "iriscaceres"
    elsewhere.mkdir(parents=True)
    (home / "p" / "iriscaceres").symlink_to(elsewhere)

    workspace = f"/private/tmp/claude-501/{_encoded(home / 'p' / 'iriscaceres')}/{SESSION}/scratchpad"
    assert canonical_workspace(workspace, home) == "[HOME]/p/iriscaceres"
