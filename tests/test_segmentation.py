...
def _session(topic_a_turns=4, topic_b_turns=4):
    ...
    for i in range(topic_a_turns):
        events.append(
            _message(len(events), "user", f"la factura de holded del trimestre {i} falta")
        )
        events.append(_message(len(events), "assistant", "reviso holded"))
    for i in range(topic_b_turns):
        events.append(
            _message(len(events), "user", f"el deploy de nova con docker fallo otra vez {i}")
        )
