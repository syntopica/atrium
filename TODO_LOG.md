...
- [x] 2026-09-08 — **Ingest / Store:** **Session scratchpads are indexed as if they were projects.** Paths like
  ...
  (`~/p/client-site -> /Volumes/archive-volume/client-site`) is one of them, because the obvious
...
- [x] 2026-09-01 — **Observability:** Both ingests report what they admitted
  ...
  - Result: new `atrium/ingest/admission_tally.py`; `to_records` takes an
    ...
    records the memstore recovery mis-filed would now be the first line of
...
- [x] 2026-08-30 — **Cutover:** memstore retired; Atrium serves Claude and
  ...
  - Result: The blocker was never synthesis coverage — it was freshness.
    ...
    embedder) replaced the memstore SessionStart hook, and an MCP server with
...
- [x] 2026-08-27 — **Cross-project (memstore):** Retirement calls executed
  ...
  - Evidence: `memstore_delete_drawers` dry-run and delete results;
    read-only re-scan count 0; `~/p/memstore/TODO.md` updated.
