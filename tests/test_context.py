...
def test_notes_are_not_starved_by_global_top_n(corpus):
    ...
    assert any(item["note_path"] == "projects/server-a/access.md" for item in result["evidence"])
...
def test_invalid_bounds(corpus, options):
    with pytest.raises(ValueError):
        retrieve_context(corpus[0], "Server-a", lane=options.pop("lane", "words"), **options)
...
def test_budget_and_match_centered_excerpt(corpus):
    ...
    limited = retrieve_context(
        connection, "Server-a", project=project, lane="words", limit=1, max_chars=1, state=state
    )
...
def test_missing_invalid_and_stale_refresh(corpus, stamp, status):
    ...
    result = retrieve_context(connection, "Server-a", project=project, lane="words", state=state)
...
def test_empty_and_broken_stores_are_explicit(tmp_path):
    ...
    result = retrieve_context(connection, "Server-a", lane="words", state=tmp_path)
    assert result["evidence"] == []
    assert result["index_status"] == "empty"
    assert result["degraded"]
    connection.close()
    broken = sqlite3.connect(":memory:")
    result = retrieve_context(broken, "Server-a", lane="words", state=tmp_path)
...
def test_outside_repository_does_not_widen_scope(corpus, tmp_path):
    with pytest.raises(ValueError, match="repository"):
        retrieve_context(corpus[0], "Server-a", project=tmp_path, lane="words")


def test_unsafe_links_cycles_and_third_party_targets(corpus):
    connection, project, state, _ = corpus
    path = "safe.md"
    text = "Server-a unsafe links [[../../outside]] [[/etc/passwd]] [[https://evil.invalid/a]] [[source]] [[safe]] [valid](projects/server-a/runbook.md)"
    with connection:
        write_conversation(
    ...
    result = retrieve_context(
        connection, "Server-a unsafe links", project=project, lane="words", state=state
    )
    ...
    assert any(item["note_path"] == "projects/server-a/runbook.md" for item in result["evidence"])
    assert not any(
        item["note_path"] == "projects/server-a/second-hop.md" for item in result["evidence"]
    )
